...
export default function claudeBridge(): Plugin {
  return {
    ...
    configureServer(server: ViteDevServer) {
      ...
      // POST /api/voiceover/generate — run /refine-voiceover skill via Claude Code CLI
      server.middlewares.use('/api/voiceover/generate', async (req, res) => {
        ...
        try {
          ...
          // Use the /refine-voiceover skill via claude -p
          const prompt =
            `/refine-voiceover\n\n` +
            `Explainer slug: ${body.slug}\n` +
            `Panel ID: ${body.panelId}\n\n` +
            `Read the panel metadata, read the panel component if it exists, ` +
            `and return ONLY the voiceover narration text — nothing else.`
